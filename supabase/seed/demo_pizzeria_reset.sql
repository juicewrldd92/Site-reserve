-- =============================================================================
-- Remise à zéro de la démo + rechargement propre
--
-- Les textes chargés précédemment sont illisibles (« ≈íufs », « Cr√®merie ») :
-- ils ont traversé un presse-papier macOS sans locale UTF-8, qui a relu les
-- octets UTF-8 comme du Mac OS Roman.
--
-- PostgreSQL ne connaît pas l'encodage Mac OS Roman, donc pas de réparation en
-- base possible sans écrire une table de conversion à la main. Comme il s'agit
-- de données de démonstration, on les efface et on les recharge — c'est plus
-- court et sans risque pour le reste.
--
-- Ne touche QUE les lignes portant une signature d'abîmage (√ ≈ ¬ Â Ã).
-- Les produits que tu as saisis toi-même dans l'app ne sont pas concernés.
-- =============================================================================

-- 1. Les lignes de stock partent avec leurs produits (cascade).
delete from public.products
 where name ~ '[√≈¬ÂÃ]'
    or category ~ '[√≈¬ÂÃ]'
    or brand ~ '[√≈¬ÂÃ]';

-- 2. Les emplacements abîmés dans les établissements.
update public.establishments e
   set locations = coalesce(sub.propres, array[]::text[])
  from (
    select e2.id,
           array_agg(l order by l) filter (where l !~ '[√≈¬ÂÃ]') as propres
      from public.establishments e2, unnest(e2.locations) as l
     group by e2.id
  ) sub
 where e.id = sub.id;

-- 3. Et dans les lignes de stock restantes.
